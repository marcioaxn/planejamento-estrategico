...
class CalculoLivewire extends Component
{
            ...
            if (isset($cod_organizacao) && !is_null($cod_organizacao) && $cod_organizacao != '') {
                ...
                foreach ($planosAcao as $planoAcao) {
                    ...
                    if (isset($cod_plano_de_acao) && !is_null($cod_plano_de_acao) && $cod_plano_de_acao != '' && isset($anoSelecionado) && !is_null($anoSelecionado) && $anoSelecionado != '') {
                        ...
                        $indicadores = Indicador::where('cod_plano_de_acao', $cod_plano_de_acao)->with('metaAno');
    ...
    public function calcularAcumuladoIndicadoresObjetivoEstrategico($cod_organizacao = '', $cod_objetivo_estrategico = '', $anoSelecionado = '')
    {
        ...
        if (isset($cod_objetivo_estrategico) && !empty($cod_objetivo_estrategico) && isset($anoSelecionado) && !empty($anoSelecionado)) {
            ...
            if ($anoSelecionado != date('Y')) {
                $date = new DateTime();
                $date->modify('-1 month');
                $mesAnterior = $date->format('m');
            }

            $indicadores = Indicador::where('cod_objetivo_estrategico', $cod_objetivo_estrategico)->with('metaAno');
            ...
            foreach ($indicadores as $indicador) {
                            ...
                            if ($mesAnterior != 12) {
                                if ($evolucaoIndicador->num_mes <= $mesAnterior) {
                                    if ($indicador->bln_acumulado === 'Sim') {
                                        ...
                                        // echo 'Valor realizado em ' . $evolucaoIndicador->num_mes . ' ' . $evolucaoIndicador->vlr_realizado . '<br />';
                ...
                // print ('Total realizado, previsto anual > ' . $totalRealizado . " - " . $totalPrevistoAnual . ' - ' . $totalResultado . "<br>");
            }

            if ($indicadores->count() > 0) {
                // print ("Quantidade de Indicadores: " . $indicadores->count() . "<br>");
            ...
            if ($indicadores->count() > 0) {
                $prc_alcancado = $resultadoGeralCalculo / $indicadores->count();
            } else {
                $prc_alcancado = 0;
            }
            ...
            $resultado['quantidadeIndicador'] = $indicadores->count();
            $resultado['percentual_alcancado'] = round($prc_alcancado, 0);
                ...
                foreach ($planosAcao as $planoAcao) {
                    ...
                    if (isset($cod_plano_de_acao) && !is_null($cod_plano_de_acao) && $cod_plano_de_acao != '' && isset($anoSelecionado) && !is_null($anoSelecionado) && $anoSelecionado != '') {
                        ...
                        foreach ($indicadores as $indicador) {
                            ...
                            // print($totalRealizado." - ".$totalPrevistoAnual . ' - ' . $totalResultado ."<br>");
                        }
    ...
    public function calcularAcumuladoIndicador($cod_indicador = '', $anoSelecionado = '')
    {
        ...
        if (isset($cod_indicador) && !is_null($cod_indicador) && $cod_indicador != '' && isset($anoSelecionado) && !is_null($anoSelecionado) && $anoSelecionado != '') {
            ...
            if (!is_null($indicador)) {
                $evolucaoIndicador = EvolucaoIndicador::where('cod_indicador', $cod_indicador)->where('num_ano', $anoSelecionado)->orderBy('num_mes')->get();
