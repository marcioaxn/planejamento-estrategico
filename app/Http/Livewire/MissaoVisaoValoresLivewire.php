...
class MissaoVisaoValoresLivewire extends Component
{
    ...
    public function render()
    {
        ...
        if(isset($this->cod_pei) && !is_null($this->cod_pei) && $this->cod_pei != '') {
            ...
            $consultarMissaoVisaoValoresPeloPei = MissaoVisaoValores::where('cod_pei',$this->cod_pei)
            ->get();

            if($consultarMissaoVisaoValoresPeloPei) {

                foreach($consultarMissaoVisaoValoresPeloPei as $result) {

                    $cods_organizacao = $cods_organizacao.$result->cod_organizacao.',';

                }

            }

            $cods_organizacao = trim($cods_organizacao,',');

            $cods_organizacao = explode(',', $cods_organizacao);
        ...
        $missaoVisaoValores = MissaoVisaoValores::orderBy('dsc_missao','desc')
        ...
        return view('livewire.missaoVisaoValores');
    }
