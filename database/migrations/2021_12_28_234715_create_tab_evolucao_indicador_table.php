...
class CreateTabEvolucaoIndicadorTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('pei.tab_evolucao_indicador', function (Blueprint $table) {
            ...
            $table->decimal('vlr_previsto', $precision = 1000, $scale = 2);
            $table->decimal('vlr_realizado', $precision = 1000, $scale = 2);
            $table->text('txt_avaliacao')->nullable(false);
            $table->string('bln_atualizado')->nullable(false);
            $table->softDeletes();
            $table->timestamps();
        });
    }
