                    ...
                    @foreach ($resultPerspectiva->objetivosEstrategicos as $resultObjetivosEstrategicos)
                        ...
                        @foreach ($resultObjetivosEstrategicos->planosDeAcao as $resultPlanoDeAcao)
                            <div
                                ...
                                <div class="bg-gray-100 bg-opacity-50 text-gray-900 text-lg px-1 pb-1 pl-3 pr-3"><span
                                        class="text-sm">{{ $resultPlanoDeAcao->tipoExecucao->dsc_tipo_execucao }}
                                        {!! $resultPerspectiva->num_nivel_hierarquico_apresentacao !!}.{!! $resultObjetivosEstrategicos->num_nivel_hierarquico_apresentacao !!}.{!! $resultPlanoDeAcao->num_nivel_hierarquico_apresentacao !!}.
                                    </span><strong>{!! $resultPlanoDeAcao->dsc_plano_de_acao !!}</strong></div>

                                <p class="px-3 px-2 pt-3"><strong>Indicadores</strong> (<span
                                        class="text-blue-400">{!! $resultPlanoDeAcao->indicadores->count() !!}</span>)</p>
